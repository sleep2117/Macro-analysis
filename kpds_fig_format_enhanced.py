...
def get_kpds_color(index):
    ...
    return KPDS_COLORS[index % len(KPDS_COLORS)]
...
def create_five_year_comparison_chart(df, title=None, y_title=None, x_axis_type='month', recent_years=3):
    ...
    if y_title:
        ...
        fig.add_annotation(
            text=y_title,
            xref="paper", yref="paper",
            x=x_pos, y=1.15,  # legend 위치를 고려해 더 위로
            showarrow=False,
...
def df_dual_axis_chart(df, left_cols=None, right_cols=None, 
    ...
    # Y축 제목들 (자동 위치 계산으로 잘림 방지)
    if left_title is not None and left_title != '':
    ...
    if right_title is not None and right_title != '':
