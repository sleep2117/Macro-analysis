...
print("주요 사용법:")
print("기본 함수들:")
print("• list_available_series()  # 사용 가능한 시리즈 목록")
print("• print_load_info()  # 데이터 로드 정보 출력")
print("• show_series_summary()  # 시리즈 요약 정보")
print("")
print("데이터 추출:")
print("• get_raw_data(['cs_national_sa', 'fhfa_national_sa'])  # 원본 데이터")
print("• get_yoy_data(['cs_national_sa'])  # 전년동월대비")
print("• get_mom_data(['cs_national_sa'])  # 전월대비")
print("")
print("시각화:")
print("• plot_multi_line_chart(['cs_national_sa', 'fhfa_national_sa'], 'yoy')")
print("• plot_dual_axis_chart('cs_national_sa', 'nar_existing_sales', 'yoy', 'raw')")
print("• plot_horizontal_bar_chart(['cs_national_sa'], 'mom', periods=12)")
print("• create_house_price_dashboard('yoy')  # 주요 지수 대시보드")
print("")
print("기존 함수들:")
...
"""
=== 추가 시각화 및 분석 함수들 ===
CPS_employ.py와 동일한 구조로 구현된 범용 함수들
"""

def list_available_series():
    """로드된 데이터에서 사용 가능한 시리즈 목록 반환"""
    if HOUSE_PRICE_DATA is None:
        print("❌ 데이터가 로드되지 않았습니다. load_all_house_price_data_enhanced()를 먼저 실행하세요.")
        return []
    
    return list(HOUSE_PRICE_DATA['raw'].columns)

def print_load_info():
    """현재 로드된 데이터의 상세 정보 출력"""
    if HOUSE_PRICE_DATA is None:
        print("❌ 데이터가 로드되지 않았습니다.")
        return
    
    raw_data = HOUSE_PRICE_DATA['raw']
    print("=" * 50)
    print("📊 House Price Data Load Information")
    print("=" * 50)
    print(f"시리즈 개수: {len(raw_data.columns):,}개")
    print(f"데이터 포인트 수: {len(raw_data):,}개")
    print(f"데이터 기간: {raw_data.index.min().strftime('%Y-%m-%d')} ~ {raw_data.index.max().strftime('%Y-%m-%d')}")
    print(f"로드 시간: {HOUSE_PRICE_DATA.get('load_time', 'Unknown')}")
    print(f"YoY 계산 완료: {'✓' if 'yoy' in HOUSE_PRICE_DATA else '✗'}")
    print(f"MoM 계산 완료: {'✓' if 'mom' in HOUSE_PRICE_DATA else '✗'}")
    print("=" * 50)
...
def get_raw_data(series_names=None):
    ...
    # 존재하지 않는 시리즈 확인
    missing_series = [s for s in series_names if s not in raw_data.columns]
    if missing_series:
        print(f"⚠️ 다음 시리즈를 찾을 수 없습니다: {missing_series}")
        series_names = [s for s in series_names if s in raw_data.columns]
    
    if not series_names:
        print("❌ 유효한 시리즈가 없습니다.")
        return None
    
    return raw_data[series_names]

def get_yoy_data(series_names=None):
    """전년동월대비 변화 데이터 반환"""
    if HOUSE_PRICE_DATA is None or 'yoy' not in HOUSE_PRICE_DATA:
        print("❌ YoY 데이터가 없습니다. load_all_house_price_data_enhanced()를 먼저 실행하세요.")
        return None
    
    yoy_data = HOUSE_PRICE_DATA['yoy']
    ...
    # 존재하지 않는 시리즈 확인
    missing_series = [s for s in series_names if s not in yoy_data.columns]
    if missing_series:
        print(f"⚠️ 다음 시리즈를 찾을 수 없습니다: {missing_series}")
        series_names = [s for s in series_names if s in yoy_data.columns]
    
    if not series_names:
        print("❌ 유효한 시리즈가 없습니다.")
        return None
    
    return yoy_data[series_names]

def get_mom_data(series_names=None):
    """전월대비 변화 데이터 반환"""
    if HOUSE_PRICE_DATA is None or 'mom' not in HOUSE_PRICE_DATA:
        print("❌ MoM 데이터가 없습니다. load_all_house_price_data_enhanced()를 먼저 실행하세요.")
        return None
    
    mom_data = HOUSE_PRICE_DATA['mom']
    ...
    # 존재하지 않는 시리즈 확인
    missing_series = [s for s in series_names if s not in mom_data.columns]
    if missing_series:
        print(f"⚠️ 다음 시리즈를 찾을 수 없습니다: {missing_series}")
        series_names = [s for s in series_names if s in mom_data.columns]
    
    if not series_names:
        print("❌ 유효한 시리즈가 없습니다.")
        return None
    
    return mom_data[series_names]
...
def plot_multi_line_chart(series_names, data_type='raw', title=None, ytitle=None, labels=None, 
    ...
    # 자동 제목 생성
    if title is None:
        data_type_names = {'raw': 'House Price Levels', 'yoy': 'House Price YoY Changes', 'mom': 'House Price MoM Changes'}
        title = data_type_names.get(data_type, 'House Price Chart')
...
def plot_dual_axis_chart(left_series, right_series, left_data_type='raw', right_data_type='raw',
    ...
    # Y축 제목 자동 설정
    if left_ytitle is None:
        ytitle_map = {'raw': 'Index Level', 'yoy': 'YoY Change (%)', 'mom': 'MoM Change (%)'}
        left_ytitle = ytitle_map.get(left_data_type, '')
    
    if right_ytitle is None:
        ytitle_map = {'raw': 'Index Level', 'yoy': 'YoY Change (%)', 'mom': 'MoM Change (%)'}
        right_ytitle = ytitle_map.get(right_data_type, '')
    
    # 자동 제목 생성
    if title is None:
        title = "House Price Dual Axis Chart"
...
def plot_horizontal_bar_chart(series_names, data_type='raw', title=None, periods=12, labels=None):
    ...
    if title is None:
        data_type_names = {'raw': 'House Price Levels', 'yoy': 'House Price YoY Changes', 'mom': 'House Price MoM Changes'}
        title = f"{data_type_names.get(data_type, 'House Price')} - Recent {periods} Months"
...
def create_house_price_dashboard(data_type='yoy'):
    """
    주요 주택가격 지수들의 대시보드 생성
    
    Parameters:
    - data_type: 'raw', 'yoy', 'mom' 중 선택
    """
    key_series = ['cs_national_sa', 'fhfa_national_sa', 'zillow_us']
    labels = ['Case-Shiller National', 'FHFA National', 'Zillow US']
    
    plot_multi_line_chart(
        series_names=key_series,
        data_type=data_type,
        title=f"Major House Price Indices ({data_type.upper()})",
        labels=labels,
        width_cm=14,
        height_cm=9
    )
...
def show_series_summary():
    ...
    print("📋 House Price Series Summary")
    print("=" * 60)
    
    # Case-Shiller 시리즈 카운트
    cs_series = [col for col in raw_data.columns if col.startswith('cs_')]
    print(f"Case-Shiller Series: {len(cs_series)}개")
    
    # FHFA 시리즈 카운트  
    fhfa_series = [col for col in raw_data.columns if col.startswith('fhfa_')]
    print(f"FHFA Series: {len(fhfa_series)}개")
    
    # Zillow 시리즈 카운트
    zillow_series = [col for col in raw_data.columns if col.startswith('zillow_')]
    print(f"Zillow Series: {len(zillow_series)}개")
    
    # NAR 시리즈 카운트
    nar_series = [col for col in raw_data.columns if col.startswith('nar_')]
    print(f"NAR Series: {len(nar_series)}개")
    ...
    # 각 카테고리별 최근 10개 시리즈 표시
    print("\n주요 시리즈별 최근값:")
    key_series = ['cs_national_sa', 'fhfa_national_sa', 'zillow_us', 'nar_existing_sales']
    for series in key_series:
        if series in raw_data.columns:
            latest_value = raw_data[series].iloc[-1]
            print(f"  {series}: {latest_value:.2f}")
